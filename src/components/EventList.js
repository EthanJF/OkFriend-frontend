...
export default class EventList extends Component {
    render(){
        const eventList = this.props.allEvents.map((event) => {
            ...
            return <li onClick={()=>this.props.showEventDetail(event.id)}>{event.name} with {otherFriend ? otherFriend.username : ""} on {readableDate} <button onClick={() => this.props.deleteEvent(event.id)}>Delete</button></li>
        })
        return(
